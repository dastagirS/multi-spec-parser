  ...
  while (index < parser->count) {
    ...
    if (frame->sequence) {
      ...
      if (ny_find_colon(rest, rest_length, false, &colon)) {
        ...
        size_t mapping_indent = line->indent + 2; bool child_frames_added = false;
        if (value_length > 0 && (value_source[0] == '|' || value_source[0] == '>')) { if (!ny_block_scalar(parser, mapping_indent, value_source, value_length, &value)) { free(key); free(frames); ny_free_node(root); return false; } index = parser->index; }
        else if (value_length == 0) {
          size_t next = 0;
          if (ny_indented_plain_start(parser, index + 1, mapping_indent, &next)) {
            if (!ny_indented_plain(parser, next, mapping_indent, &value, &index)) { free(key); free(frames); ny_free_node(root); return false; }
          } else if (ny_next_content(parser, index + 1, &next) && parser->lines[next].indent > mapping_indent) {
            ...
            frames[frame_count++] = (NyFrame){map, mapping_indent, false};
            frames[frame_count++] = (NyFrame){value, parser->lines[next].indent, value->type == NY_SEQUENCE};
            child_frames_added = true;
          } else value = ny_node(parser, NY_NULL);
        }
        else { char *joined = NULL; size_t joined_length = 0; if (value_length > 0 && value_source[0] == '\'' && !ny_single_quote_complete(value_source, value_length) && !ny_join_single_quoted(parser, &index, value_source, value_length, &joined, &joined_length)) { free(key); free(frames); ny_free_node(root); return false; } if (joined == NULL && value_length > 0 && value_source[0] == '"' && !ny_double_quote_complete(value_source, value_length) && !ny_join_double_quoted(parser, &index, value_source, value_length, &joined, &joined_length)) { free(key); free(frames); ny_free_node(root); return false; } if (joined == NULL && value_length > 0 && value_source[0] != '\'' && value_source[0] != '"' && !ny_is_flow(value_source, value_length) && !ny_join_plain(parser, &index, mapping_indent, value_source, value_length, &joined, &joined_length)) { free(key); free(frames); ny_free_node(root); return false; } if (joined == NULL && ny_is_flow(value_source, value_length) && !ny_join_flow(parser, &index, value_source, value_length, &joined, &joined_length)) { free(key); free(frames); ny_free_node(root); return false; } if (joined != NULL) { value = NULL; if (!ny_inline(parser, joined, joined_length, &value)) { free(joined); free(key); free(frames); ny_free_node(root); return false; } free(joined); } else if (!ny_inline(parser, value_source, value_length, &value)) { free(key); free(frames); ny_free_node(root); return false; } }
        if (value == NULL || !ny_map_add(parser, map, key, value)) { free(key); free(frames); ny_free_node(root); return false; }
        if (!child_frames_added) { size_t next = 0; if (ny_next_content(parser, index + 1, &next) && parser->lines[next].indent == mapping_indent) { if (frame_count >= parser->max_depth) { free(frames); ny_free_node(root); return ny_fail(parser, "nesting limit exceeded"); } frames[frame_count++] = (NyFrame){map, mapping_indent, false}; } }
        index++; continue;
      }
      NyNode *value = NULL;
